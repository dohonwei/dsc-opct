...
OUT = ROOT / "outputs/dcs_opct_v11_submission_artifacts_crossfit_v19"
...
def collect_sources() -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing v19 package sources: {missing}")
    return dict(sorted(sources.items()))


def main() -> None:
    if OUT.exists():
        raise FileExistsError(f"Immutable v19 package already exists: {OUT}")
    ...
    progress = tqdm(sources.items(), desc="Build DCS-OPCT v19 package", unit="file", dynamic_ncols=True)
    ...
    manifest = {
        "schema_version": "dcs-opct-v11-bspc-submission-package-v19",
    ...
    print(f"DCS-OPCT v19 package built: {len(hashes)} files")
    print(f"Package: {OUT}")
