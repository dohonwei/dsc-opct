...
def main() -> None:
    ...
    record("schema_version", manifest.get("schema_version") == "dcs-opct-v11-bspc-submission-package-v19", manifest.get("schema_version"))
    ...
    progress = tqdm(packaged.items(), desc="Validate DCS-OPCT v19 hashes", unit="file", dynamic_ncols=True)
    ...
    if report["status"] != "passed":
        ...
        raise RuntimeError(f"DCS-OPCT v19 package validation failed: {failed}")
    print(f"DCS-OPCT v19 package validation passed: {passed}/{len(checks)} checks")
