...
OUTPUT_ROOT = ROOT / "outputs/avdos_v11_external_confirmation"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply frozen DCS-OPCT v11 to AVDOS once.")
...
def load_unlabeled_target(root: Path) -> pd.DataFrame:
    ...
    if table.model_capacity.isna().any() or len(table) != 320:
        raise ValueError(f"AVDOS unlabeled risk table must contain 320 rows, observed {len(table)}")
...
def main() -> None:
    ...
    action_lock = {
        "status": "avdos_v11_action_and_audit_locked_before_outcomes",
    ...
    gate = {
        ...
        "interpretation": (
            "DCS-OPCT v11 passed untouched AVDOS effectiveness and safety gates"
            if claim_supported
            else (
                "safe abstention without evidence of external effectiveness"
                if selected == "identity" and safety_pass
                else "untouched AVDOS confirmation did not support the target claim"
            )
        ),
    }
    ...
    manifest = {
        "status": "avdos_v11_one_shot_external_confirmation_complete",
        "date": "2026-09-08",
        "claim_supported": claim_supported,
