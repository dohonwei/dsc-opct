...
def collect_sources() -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing v24 package sources: {missing}")
    return dict(sorted(sources.items()))


def main() -> None:
    if OUT.exists():
        raise FileExistsError(f"Immutable v24 package already exists: {OUT}")
    ...
    progress = tqdm(sources.items(), desc="Build DCS-OPCT v24 package", unit="file", dynamic_ncols=True)
    ...
    manifest = {
        "schema_version": "dcs-opct-v11-bspc-submission-package-v24",
    ...
    print(f"DCS-OPCT v24 package built: {len(hashes)} files")
    print(f"Package: {OUT}")
