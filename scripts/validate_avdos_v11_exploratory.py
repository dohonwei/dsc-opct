...
CONFIRMATION_ROOT = ROOT / "outputs/avdos_v11_exploratory_ppg"
...
def main() -> None:
    ...
    exploratory_implementation = json.loads(
        EXPLORATORY_LOCK.read_text(encoding="utf-8")
    )
    ...
    exploratory_hashes = {
        relative: sha256(ROOT / relative)
        for relative in exploratory_implementation["analysis_code_sha256"]
    }
    checks.append(
        check(
            "exploratory_implementation_hashes",
            exploratory_hashes
            == exploratory_implementation["analysis_code_sha256"],
            exploratory_hashes,
        )
    )
    checks.append(
    ...
    trial_contract = bool(
        ...
        and trials.groupby("subject_id").size().eq(6).all()
    ...
    gpu_contract = bool(
        ...
        and settings["tasks"] == ["arousal"]
        and settings["axes"] == ["subject"]
    ...
    dose_contract = bool(
        len(risk_rows) == 160
        and set(risk_rows.task) == {"arousal"}
        and len(identity) == 40
    )
    checks.append(
        check(
    ...
    assignment_contract = bool(
        assignments.partition.value_counts().to_dict() == {"audit": 20, "heldout": 20}
    ...
    checks.append(
        check(
            ...
            == sha256(FEATURE_ROOT / "avdos_trial_features.csv")
            and confirmation_manifest["confirmatory_claim_supported"] is False
            and gate["confirmatory_claim_supported"] is False,
            gate,
        )
    )
