...
def collect_sources() -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing v21 package sources: {missing}")
    return dict(sorted(sources.items()))


def main() -> None:
    if OUT.exists():
        raise FileExistsError(f"Immutable v21 package already exists: {OUT}")
    ...
    progress = tqdm(sources.items(), desc="Build DCS-OPCT v21 package", unit="file", dynamic_ncols=True)
    ...
    manifest = {
        "schema_version": "dcs-opct-v11-bspc-submission-package-v21",
    ...
    print(f"DCS-OPCT v21 package built: {len(hashes)} files")
    print(f"Package: {OUT}")
