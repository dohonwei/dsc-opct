...
from emognition_v11_schema_adapter import load_emognition_task_bundles
...
IMPLEMENTATION_LOCK = ROOT / "docs/emognition_v11_external_confirmation_implementation_lock.json"
OUTPUT_ROOT = ROOT / "outputs/emognition_v11_external_confirmation"
RISK_ROOT = ROOT / "outputs/identity_shortcut_risk_classifier"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the locked Emognition DCS-OPCT v11 one-shot GPU confirmation"
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.schema_manifest.resolve() != locked_schema:
        raise RuntimeError(
            "The requested schema manifest is not the manifest bound to the implementation lock"
        )
    if sha256(args.schema_manifest) != implementation["schema_manifest_sha256"]:
        raise RuntimeError("The locked Emognition schema manifest hash changed")
    config = application_config()
    preoutcome_sha = args.preoutcome_lock_sha256
    action_sha = args.action_lock_sha256

    if args.stage in {"prepare", "all"}:
        bundles, adapter_provenance = load_emognition_task_bundles(
        ...
        preoutcome_path, preoutcome_sha = prepare_and_lock_preoutcome(
            ...
            task_validators={
                "valence": validate_emognition_task_bundle,
                "arousal": validate_emognition_task_bundle,
            },
    ...
    print(
        "Emognition one-shot final gate: "
