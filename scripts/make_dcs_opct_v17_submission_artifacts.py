...
OUT = ROOT / "outputs/dcs_opct_v11_submission_artifacts_crossfit_v17"
...
AUTHOR_BLOCKERS = (
    "Insert an anonymous read-only repository URL or archival DOI in the manuscript.",
    "Confirm the funding statement or explicitly confirm that no specific funding applies.",
    "Confirm the official English name of the approving ethics committee against the approval document.",
    "Add the exact channels 5-9 index order only if it is recoverable from acquisition records.",
)
...
def collect_sources() -> dict[str, Path]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing v17 package sources: {missing}")
    return dict(sorted(sources.items()))


def main() -> None:
    if OUT.exists():
        raise FileExistsError(f"Immutable v17 package already exists: {OUT}")
    ...
    progress = tqdm(sources.items(), desc="Build DCS-OPCT v17 package", unit="file", dynamic_ncols=True)
    ...
    manifest = {
        "schema_version": "dcs-opct-v11-bspc-submission-package-v17",
        ...
        "external_scientific_blocker": (
            "No completed prospectively reserved compatible external dataset has passed all frozen gates "
            "and produced a non-identity release. This package therefore does not claim prospective "
            "external effectiveness or universal safety."
        ),
        "claim_boundary": (
            "The package supports retrospective selective configuration-level audit-risk calibration "
            "within evaluated development support and verifies fail-closed execution. It does not "
    ...
    print(f"DCS-OPCT v17 package built: {len(hashes)} files")
    print(f"Package: {OUT}")
