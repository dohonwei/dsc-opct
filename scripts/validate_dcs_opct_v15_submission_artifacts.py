...
def main() -> None:
    ...
    mismatches = {}
    progress = tqdm(sorted(output_hashes.items()), desc="Validate v15 package hashes", unit="file", dynamic_ncols=True)
    ...
    volatile = set(manifest.get("volatile_source_snapshots", []))
    record("volatile_snapshot_set_exact", volatile == EXPECTED_VOLATILE, sorted(volatile))
    source_mismatches = {}
    ...
    record("immutable_source_hashes_current", not source_mismatches, source_mismatches)
    ...
    v14_report = read_json(OUT / "v14_base_independent_validation_report.json")
    v14_supersession = read_json(OUT / "v14_package_superseded.json")
    ...
    record("v13_historical_package_14_of_14", v13_report.get("status") == "passed" and v13_report.get("checks_passed") == v13_report.get("checks_total") == 14, [v13_report.get("checks_passed"), v13_report.get("checks_total")])
    record("v14_source_semantics_failure_preserved", v14_report.get("status") == "failed" and v14_report.get("checks_passed") == 19 and v14_report.get("checks_total") == 20 and v14_supersession.get("successor", "").endswith("v15"), [v14_report.get("checks_passed"), v14_report.get("checks_total")])
    record("external_access_v5_12_of_12", external_report.get("status") == "passed" and external_report.get("checks_passed") == external_report.get("checks_total") == 12, [external_report.get("checks_passed"), external_report.get("checks_total")])
    record("v11_release_snapshot_passed", v11_readiness.get("status") == "passed" and all(v11_readiness.get("checks", {}).values()), v11_readiness.get("checks"))
    record("v11_inventory_snapshot_16_of_16", v11_inventory.get("status") == "passed" and v11_inventory.get("checks_passed") == v11_inventory.get("checks_total") == 16, [v11_inventory.get("checks_passed"), v11_inventory.get("checks_total")])
    record("amigos_snapshot_cuda_13_of_13", amigos.get("status") == "passed" and len(amigos.get("tests", [])) == 13 and amigos.get("checks", {}).get("cuda_available") is True, len(amigos.get("tests", [])))
    record("emognition_snapshot_cuda_15_of_15", emognition.get("status") == "passed" and len(emognition.get("tests", [])) == 15 and emognition.get("checks", {}).get("cuda_available") is True, len(emognition.get("tests", [])))
    ...
    record("no_restricted_participant_data", manifest.get("restricted_participant_data_included") is False, manifest.get("restricted_participant_data_included"))
    ...
    record("anti_cycle_rule_explicit", "not package members" in manifest.get("anti_cycle_rule", ""), manifest.get("anti_cycle_rule"))
    ...
    if report["status"] != "passed":
        ...
        raise RuntimeError(f"DCS-OPCT v15 package validation failed: {failed}")
    print(f"DCS-OPCT v15 package validation passed: {passed}/{len(checks)} checks")
