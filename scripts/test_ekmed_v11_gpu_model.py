...
from ekmed_v11_contract import GPU_EPOCHS, SEEDS
...
def main() -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required by the registered EKM-ED neural branch")
    progress = tqdm(
        SEEDS[:2],
        desc="EKM-ED synthetic CUDA fits",
        unit="fit",
        dynamic_ncols=True,
    )
    ...
    progress.close()
    print(
        "EKM-ED synthetic GPU test passed on "
