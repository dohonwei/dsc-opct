...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build registered AVDOS-VR trials.")
    parser.add_argument("--source", type=Path, default=SOURCE)
    parser.add_argument(
        "--output-root", type=Path, default=Path("outputs/avdos_trial_features")
    )
...
def verify_locks(source: Path) -> dict[str, object]:
    for path in (RESERVATION, RESERVATION_AMENDMENT, FREEZE, IMPLEMENTATION_LOCK):
        if not path.is_file():
...
def main() -> None:
    ...
    heart_ppg = tuple(
        ...
        or column.startswith("Ppg/Raw.ppg_")
    )
    wearable_all = tuple(dict.fromkeys((*HRV_COLUMNS, *heart_ppg)))
    representations = {
        "hrv": list(HRV_COLUMNS),
        "heart_ppg": list(heart_ppg),
        "minimal_wearable": list(MINIMAL_COLUMNS),
        "wearable_all": list(wearable_all),
    }
    registered_features = list(dict.fromkeys(wearable_all))
    ...
    for (participant, segment), group in progress:
        ...
        if any(len(block) == 0 for block in blocks):
            raise ValueError(f"AVDOS {participant}/{segment} cannot form three blocks")
        for block_index, indices in enumerate(blocks, start=1):
            block = group.iloc[indices]
            row = {
                "subject_id": str(participant),
                "trial_id": int(ACTIVE_SEGMENTS.index(segment) * 3 + block_index),
                "segment": segment,
    ...
    if len(trials) != 333 or trials.subject_id.nunique() != 37:
        raise ValueError("AVDOS trials violate the registered 37 x 9 contract")
    if not trials.groupby("subject_id").size().eq(9).all():
        raise ValueError("Every AVDOS participant must contribute nine trials")
    for task in ("arousal", "valence"):
        labels = (trials[f"{task}_score"].to_numpy(float) >= 5.0).astype(int)
        if len(np.unique(labels)) != 2:
            raise ValueError(f"AVDOS {task} is one-class at the registered threshold")
    ...
    manifest = {
        "status": "built_under_registered_avdos_v11_protocol",
        "dataset": "AVDOS-VR",
        "rows": len(trials),
