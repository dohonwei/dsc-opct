...
def main() -> None:
    ...
    record("schema_version", manifest.get("schema_version") == "dcs-opct-v11-bspc-submission-package-v18", manifest.get("schema_version"))
    ...
    progress = tqdm(packaged.items(), desc="Validate DCS-OPCT v18 hashes", unit="file", dynamic_ncols=True)
    ...
    if report["status"] != "passed":
        ...
        raise RuntimeError(f"DCS-OPCT v18 package validation failed: {failed}")
    print(f"DCS-OPCT v18 package validation passed: {passed}/{len(checks)} checks")
