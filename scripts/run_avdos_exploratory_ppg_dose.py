...
FEATURE_ROOT = ROOT / "outputs/avdos_exploratory_ppg_features"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run post-access exploratory AVDOS PPG dose experiment.")
    parser.add_argument(
        "--output-root",
        type=Path,
        default=ROOT / "outputs/avdos_exploratory_ppg_dose",
    )
...
def fold_assignments(frame: pd.DataFrame, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    fold_permutation = rng.permutation(STIMULUS_FOLDS)
    base_folds = {
        "Neutral": (0, 1),
        "Positive": (0, 2),
        "Negative": (1, 2),
    }
    trial_segments = frame[["trial_id", "segment"]].drop_duplicates()
    stimulus_map = {}
    for segment, registered_folds in base_folds.items():
        trial_ids = trial_segments.loc[
            trial_segments.segment.eq(segment), "trial_id"
        ].to_numpy(int)
        if len(trial_ids) != 2:
            raise ValueError(f"Expected two exploratory trials for segment {segment}")
        rng.shuffle(trial_ids)
        for trial_id, fold in zip(trial_ids, registered_folds, strict=True):
            stimulus_map[int(trial_id)] = int(fold_permutation[fold])
    return (
...
def registered_crossed_sets(
    ...
    test = np.flatnonzero(
        (subject_fold == subject_test) & (stimulus_fold == stimulus_test)
    )
    unseen = np.flatnonzero(
        (subject_fold != subject_test) & (stimulus_fold != stimulus_test)
    )
    if len(test) == 0 or len(unseen) == 0:
        raise ValueError("Registered crossed test or unseen-control quadrant is empty")
    return test, {"unseen_both": unseen}
...
def preflight(frame: pd.DataFrame, settings: dict[str, object]) -> dict[tuple, tuple]:
    ...
    for split_seed in SEEDS:
        ...
        for task in TASKS:
            labels = frame.segment.ne("Neutral").astype(int).to_numpy()
            for subject_test in range(SUBJECT_FOLDS):
                for stimulus_test in range(STIMULUS_FOLDS):
                    test, train_sets = registered_crossed_sets(
                        subject_fold,
                        stimulus_fold,
                        subject_test,
                        stimulus_test,
...
def main() -> None:
    ...
    settings = {
        ...
        "label_definition": "official AVDOS mapping: Neutral=0; Negative=1; Positive=1",
        "doses": list(DOSES),
        "candidate_draws": 256,
    ...
    manifest = {
        "status": "post_access_exploratory_avdos_counterfactual_complete",
        ...
        "exploratory_lock_sha256": file_sha256(EXPLORATORY_LOCK),
