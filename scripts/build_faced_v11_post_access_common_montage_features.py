...
DEFAULT_FEATURE_ROOT = ROOT / "outputs/faced_v11_post_access_common_montage_features"
...
BILATERAL_PAIRS = (
    ("FP1", "FP2"), ("F7", "F8"), ("F3", "F4"), ("FC5", "FC6"),
    ("FC1", "FC2"), ("T7", "T8"), ("C3", "C4"), ("CP5", "CP6"),
    ("CP1", "CP2"), ("P7", "P8"), ("P3", "P4"), ("PO3", "PO4"),
    ("O1", "O2"),
)
ALLOWED_AUXILIARY_PAIRS = (frozenset(("A1", "A2")), frozenset(("HEOR", "HEOL")))


def canonical_channel(name: str) -> str:
    normalized = normalized_channel(name)
    return LEGACY_CHANNEL_MAP.get(normalized, normalized)


def verify_exploratory_lock() -> dict[str, object]:
    if not EXPLORATORY_LOCK.is_file():
        raise RuntimeError("FACED post-access exploratory implementation lock is absent")
    lock = json.loads(EXPLORATORY_LOCK.read_text(encoding="utf-8"))
    if lock.get("status") != "post_access_exploratory_implementation_locked":
        raise RuntimeError("FACED exploratory implementation lock status is invalid")
    if lock.get("protocol_sha256") != sha256(EXPLORATORY_PROTOCOL):
        raise RuntimeError("FACED exploratory lock does not match its protocol")
    relative = "scripts/build_faced_v11_post_access_common_montage_features.py"
    if sha256(Path(__file__)) != lock["analysis_code_sha256"][relative]:
        raise RuntimeError("FACED exploratory feature builder changed after lock")
    return lock
...
def main() -> None:
    ...
    for subject_id, bdf_path, events_path, channels_path in progress:
        ...
        try:
            ...
            channel_names = tuple(canonical_channel(name) for name in channel_table.name)
            auxiliary = frozenset(set(channel_names) - set(REQUIRED_CHANNELS))
            if (
                not set(REQUIRED_CHANNELS).issubset(channel_names)
                or len(channel_names) != 32
                or auxiliary not in ALLOWED_AUXILIARY_PAIRS
            ):
                raise ValueError(
                    "channels.tsv does not match either audited FACED montage"
                )
            ...
            raw_map = {canonical_channel(name): name for name in raw.ch_names}
    ...
    manifest = {
        "status": "faced_post_access_common_montage_trial_features_built",
        "dataset": "FACED",
        ...
        "reference": "within-sample common average across the audited 30-channel common EEG montage",
        "canonical_common_channels": list(REQUIRED_CHANNELS),
        "legacy_channel_mapping": LEGACY_CHANNEL_MAP,
        "excluded_noncomparable_auxiliary_pairs": [["A1", "A2"], ["HEOR", "HEOL"]],
