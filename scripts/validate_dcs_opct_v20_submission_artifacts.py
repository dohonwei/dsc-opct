...
OUT = ROOT / "outputs/dcs_opct_v11_submission_artifacts_crossfit_v20"
...
def main() -> None:
    ...
    record("schema_version", manifest.get("schema_version") == "dcs-opct-v11-bspc-submission-package-v20", manifest.get("schema_version"))
    ...
    progress = tqdm(packaged.items(), desc="Validate DCS-OPCT v20 hashes", unit="file", dynamic_ncols=True)
    ...
    if report["status"] != "passed":
        ...
        raise RuntimeError(f"DCS-OPCT v20 package validation failed: {failed}")
    print(f"DCS-OPCT v20 package validation passed: {passed}/{len(checks)} checks")
