...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the registered FACED subject-identity dose experiment on CUDA."
    )
    parser.add_argument("--feature-root", type=Path, default=DEFAULT_FEATURE_ROOT)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_DOSE_ROOT)
...
def main() -> None:
    ...
    if args.gpu_epochs != GPU_EPOCHS:
        raise ValueError(
            f"The registered FACED protocol fixes --gpu-epochs at {GPU_EPOCHS}"
        )
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is required for the registered FACED gpu_mlp experiment"
        )
    ...
    if feature_manifest.get("status") != "faced_registered_trial_features_built":
        raise RuntimeError("FACED feature manifest is not complete")
    ...
    if any(
        ...
        raise ValueError("A registered FACED representation is empty or non-finite")
    ...
    manifest = {
        "status": "registered_faced_counterfactual_complete",
        "dataset": DATASET,
        "task": TASK,
        "axes": [AXIS],
