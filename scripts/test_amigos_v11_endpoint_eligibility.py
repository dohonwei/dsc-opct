...
from amigos_v11_contract import MATERIAL_THRESHOLD, ROOT


OUTPUT = ROOT / "outputs/amigos_v11_preaccess"
CSV = OUTPUT / "endpoint_eligibility_operating_characteristics.csv"
REPORT = OUTPUT / "endpoint_eligibility_report.json"
FIGURES = (
    OUTPUT / "amigos_v11_endpoint_eligibility.png",
    OUTPUT / "amigos_v11_endpoint_eligibility.pdf",
)
...
def main() -> None:
    ...
    checks = {
        ...
        "eligibility_monotone_in_prevalence": all(
            np.all(np.diff(group.sort_values("event_prevalence").both_halves_mixed_probability) >= -0.005)
        ...
        "material_threshold_unchanged": bool(
            report["registered_design"]["material_event_threshold"]
            == MATERIAL_THRESHOLD
        ),
        "participant_values_not_accessed": report["participant_values_accessed"]
        is False
    ...
    if failed:
        raise AssertionError(f"AMIGOS endpoint-eligibility checks failed: {failed}")
    print(f"AMIGOS endpoint-eligibility test passed: {len(checks)}/{len(checks)}")
