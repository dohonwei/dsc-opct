...
def main() -> None:
    ...
    mismatches = {}
    progress = tqdm(sorted(output_hashes.items()), desc="Validate v13 package hashes", unit="file", dynamic_ncols=True)
    ...
    if report["status"] != "passed":
        ...
        raise RuntimeError(f"DCS-OPCT v13 package validation failed: {failed}")
    print(f"DCS-OPCT v13 package validation passed: {passed}/{len(checks)} checks")
