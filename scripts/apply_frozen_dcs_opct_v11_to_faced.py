...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply frozen DCS-OPCT v11 to FACED once."
    )
    ...
    parser.add_argument("--output-root", type=Path, default=DEFAULT_CONFIRMATION_ROOT)
...
def load_unlabeled_target(root: Path) -> pd.DataFrame:
    ...
    if set(table.axis) != {"subject"}:
        raise ValueError("FACED primary confirmation is restricted to the subject axis")
...
def main() -> None:
    ...
    if args.output_root.exists():
        raise FileExistsError(
            f"Refusing to overwrite FACED confirmation: {args.output_root}"
        )
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for frozen FACED DCS-OPCT application")
    ...
    action_lock = {
        "status": "faced_v11_action_and_audit_locked_before_outcomes",
    ...
    gate = {
        ...
        "interpretation": (
            "DCS-OPCT v11 passed the untouched FACED effectiveness and safety gates"
            if claim_supported
            else (
                "identity abstention without evidence of external effectiveness"
                if selected == "identity" and safety_pass
                else "the untouched FACED confirmation did not support the target claim"
            )
        ),
    }
    ...
    manifest = {
        "status": "faced_v11_one_shot_external_confirmation_complete",
        "date": "2026-09-09",
        "claim_supported": claim_supported,
